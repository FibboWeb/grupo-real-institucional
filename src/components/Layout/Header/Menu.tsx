          ...
          <ul className="origin-top relative container top-5 flex flex-col justify-center gap-4 font-bold text-lg items-center text-black bg-white drop-shadow-md shadow-md rounded-md">
            {menuItems.map((item) => (
              <li className="flex relative gap-2 px-5 py-3" key={item.node.id}>
                ...
                {item.node?.childItems?.edges && item.node.childItems?.edges.length > 0 && (
                  <div onClick={() => handleMouseEnter(item.node.id)} className="w-full">
                    <span
                      className={`flex items-center cursor-pointer transition-all duration-300 ${activeMenu === item.node.id ? "rotate-180" : ""}`}
                    >
                      <ChevronDown />
                    ...
                    <ul
                      className={`z-10 bg-white absolute top-full rounded-md shadow-custom_shadow right-0 text-center w-96 left-0 sub-menu px-3 py-4 ${activeMenu === item.node.id ? "block opacity-100" : "hidden opacity-0"} bg-accent-neutral transition-all duration-400 left-1/2 transform -translate-x-1/2 rounded-md`}
                    >
                      {item.node.childItems?.edges?.map((subMenu) => (
