    ...
    <section className="bg-[#4FC372] py-12 px-5 sm:px-0">
      <div className="fb_container w-full flex flex-col sm:flex-row justify-between gap-8 sm:gap-0">
        {items.map((item, index) => (
          <div className="flex flex-col items-center justify-center sm:w-[25%]" key={index}>
            {item.icon && <Image src={item.icon} alt="" className="bg-white p-4 rounded-full" width={80} height={80} />}
            {item.title && <h3 className="text-white text-4xl font-bold">{item.title}</h3>}
