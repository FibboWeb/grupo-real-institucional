...
export default function FeatureSection({ backgroundClass, badge, title, content, infoSectionHeroBgImage,
    infoSectionBadge,
    infoSectionTitle,
    infoSectionContent,
    infoSectionCtaLink,
    ...
    infoSectionContentButton = "Leia mais", }: FeatureSectionProps) {
    return (
        <div className="relative">
            <div className={`${backgroundClass} absolute left-0 right-0 top-0 bottom-0 bg-cover md:bg-contain bg-no-repeat`}></div>
                    ...
                    <div className="flex flex-col items-center">
                        {badge && (
                            <div className="flex gap-1 py-1 w-fit">
                                ...
                                <div className={`text-xl ${backgroundClass == "bg-green-featured" ? 'text-white' : 'text-fb_green'} text-uppercase font-semibold`}>{badge}</div>
                            </div>
                        )}
                        {title && <h2 className={`font-semibold text-3xl ${backgroundClass == "bg-green-featured" ? 'text-white' : 'text-[var(--blue-main)]'} w-fit`}>{title}</h2>}
