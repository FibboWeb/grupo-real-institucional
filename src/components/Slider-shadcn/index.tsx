...
export function ShadcnCarroussel({ itens }: any) {
  ...
  return (
    <div className="flex overflow-hidden flex-col md:flex-row w-full gap-1">
          ...
          <CarouselContent className="ml-0 flex gap-12">
            ...
            {itens.map((post, index) => (
              <CarouselItem key={index} className="md:basis-1/2 lg:basis-1/4">
                <div className="p-1 w-[372px] mx-auto">
                  <CardBlog
                    key={post.id}
  ...
  return (
    <div className="flex overflow-hidden flex-col md:flex-row w-full gap-1">
      <Carousel
        ...
        className="w-full md:ml-[30px] lg:ml-[100px]"
      >
        <CarouselContent className="ml-0 flex gap-12"> {/* Adicione um gap para espaçamento */}
          <CarouselItem className="py-2 w-[280px] sm:w-[300px] ml-">
            <div className="flex flex-col justify-between min-h-[440px] rounded-2xl bg-fb_gradient text-white p-6">
              <div className="content">
                <h2 className="text-3xl font-bold">Notícias</h2>
                <p className="pt-6">
          ...
          {itens.map((post, index) => (
            <CarouselItem key={index} className="basis-1/4 xl:basis-1/6 flex py-2 min-w-[280px] sm:min-w-[300px]">
              <div className="w-72 md:w-[372px] mx-auto">
                <CardBlog
                  key={post.id}
                  blogContext={post.categories.nodes[0].name.toLowerCase() === "artigos" ? "/artigos" : "/noticias"}
                  postImage={post.featuredImage.node.sourceUrl}
...
export function CarouselEmbla({ itens }: any) {
    ...
    return (
      <div className="flex flex-col lg:flex-row gap-6 xl:gap-10 md:pl-[30px] lg:pl-[100px]">
        <Carousel
        opts={{
          align: "start",
          loop: true,
        ...
        >
          <div className="flex ">
            <CarouselContent className="">
              <div className="py-2 z-10 hidden lg:block">
